...
// Use same user id as logged-in web app user
const EXTENSION_USER_ID = "28bc5ff1-952c-4da8-8338-e5293470a245";
...
async function loadCurrentUrl() {
  ...
  if (saved[currentUrl]) {
    currentKbId = saved[currentUrl].kb_id;
    currentChatId = saved[currentUrl].chat_id;
    setStatus("Existing chat loaded for this page.");
  }
}
...
async function ingestPage() {
  ...
  try {
    ...
    if (!ingestData.success || !ingestData.kb_id) {
      setStatus("Ingestion failed.");
      answerBox.textContent = JSON.stringify(ingestData, null, 2);
      return;
    }
    ...
    setStatus("Page ingested. Chat saved.");
  } catch (error) {
    console.error(error);
    setStatus("Error during ingestion.");
    answerBox.textContent = error.message;
...
async function askQuestion() {
  ...
  try {
    ...
    setStatus("Answer saved to chatbot history.");
  } catch (error) {
    console.error(error);
    setStatus("Error while asking question.");
    answerBox.textContent = error.message;
